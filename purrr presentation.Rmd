...
Run your analysis among all patients, and again excluding low grade patients
...
1. store vector of the files you want to import
```{r, echo=TRUE, eval=FALSE}
    file_list = list.files(pattern = "*.csv")
```

2. use `map()` to read the files. returns a list where each element is a tibble  
```{r, echo=TRUE, eval=FALSE}
    map(file_list, read_csv)
```

3. to append each of the datasets, use the `map_dfr()` function
    - after each file has been imported, `bind_rows()` will create one final tibble
```{r, echo=TRUE, eval=FALSE}
    map_dfr(file_list, read_csv)
```

4. to include an identifier, add a `mutate()` 
    - add a column called file indicating the file the rows originate
```{r, echo=TRUE, eval=FALSE}
    map_dfr(file_list, ~read_csv(.x) %>% mutate(file = .x))
```
    - or assign each file an id
```{r, echo=TRUE, eval=FALSE}
    map2_dfr(file_list, 1:length(file_list) ~read_csv(.x) %>% mutate(file_id = .y))
```
...
- we'll use 200 resampled datasets to estimate the standard deviation of the response rate difference 
...
# bootstrap analyses
```{r, eval=TRUE, echo=TRUE}
# resample dataset 200 times and calculate risk difference
risk_diff_boot <- map_dbl(
  1:200,
  ~ trial %>% 
    slice(sample.int(200, replace = TRUE)) %>%
    risk_diff()
  ) 
```
...
- `pluck()`, and `chuck()`
- `cross()`, `cross2()`, `cross3()`, and `cross_df()`

as an added bonus Lauren Hill is here.
