...
base apply is to *Sister Act* as purrr map is to ***Sister Act 2: Back in the Habit*, the sequal to a great film that somehow is even better than the first!**
...
---
# subgroup analysis
purrr::map_dbl + tidyr::nest + dplyr::mutate
```{r, echo=TRUE, eval=FALSE}
trial %>%
  group_by(grade) %>%
  nest() %>%
...
we can coerce the output to by any type
