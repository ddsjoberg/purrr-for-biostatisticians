...
output:
  xaringan::moon_reader:
    css: [ninjutsu, default, default-fonts]
    lib_dir: libs
    nature:
...
.pull-left[
- the `map*()` functions have greater consistency among functions

- first argument to `lapply()` is the data; the first argument to `mapply()` is the function
  
- with `v-`, `s-`, and `mapply()` uses `USE.NAMES = FALSE` to suppress names in output; `lapply()` does not have this argument
]
...
- output from `*apply()` is not consistent; output from `map*()` is entirely predictable
]
